...
static uint32_t e2nxx_dtb_compatible(void *dtb, struct msm_id *devid, struct msm_id *dtb_id)
{
    int root_offset;
    const void *prop;
    int len;
...
static int e2nxx_choose_dtb(const char *dtb_img, off_t dtb_len, char **dtb_buf, off_t *dtb_length)
{
    ...
    while(dtb + sizeof(struct fdt_header) < dtb_end)
    {
        ...
        if(e2nxx_dtb_compatible(dtb, &devid, &dtb_id))
        {
...
static int e2nxx_add_extra_regs(void *dtb_buf)
{
    FILE *f;
    uint32_t reg;
    int res;
...
const struct arm_mach arm_mach_e2nxx = {
    .boardnames = { "e2nac", "e2nam", "e2nas", "e2nav", NULL },
    .choose_dtb = e2nxx_choose_dtb,
    .add_extra_regs = e2nxx_add_extra_regs,
};
